...
namespace FreeDraw
{
    ...
    public class Drawable : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (PenSpriteRef.enabled)
            {
                mouse_held_down = true;
                Pen_Colour = PenSpriteRef.gameObject.GetComponent<StoryScopePen>().PenColor;
            ...
            else if (GumSpriteRef.enabled)
            {
                ...
                Pen_Width = GumSpriteRef.gameObject.GetComponent<StoryScopeGum>().PenWidth;
                //Debug.Log(Pen_Colour);
            }
            else
            {
            ...
            if (mouse_held_down)
            {
                ...
                Collider2D hit = Physics2D.OverlapPoint(PenPos, Drawing_Layers.value);
                if (hit != null && hit.transform != null)
                {
                    // We're over the texture we're drawing on!
                    // Use whatever function the current brush is
                    //Debug.Log("Draw");
                    current_brush(PenPos);
                }
